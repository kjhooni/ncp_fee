...
def func(method, api_server, api_url, body):
    message = (
        method
        + var.space
        + api_url
        ...
        + var.timestamp
        + var.new_line
        + api_key.ncloud_accesskey
    )

    message = bytes(message, "UTF-8")

    ncloud_secretkey2 = bytes(api_key.ncloud_secretkey, "UTF-8")
    ...
    if method == "GET":
        ...
        http_header = {
            ...
            "x-ncp-iam-access-key": api_key.ncloud_accesskey,
...
def func_gov(method, api_server, api_url, body):
    message = (
        method
        + var.space
        + api_url
        ...
        + var.timestamp
        + var.new_line
        + api_key.ncloud_gov_accesskey
    )

    message = bytes(message, "UTF-8")

    ncloud_secretkey2 = bytes(api_key.ncloud_gov_secretkey, "UTF-8")
    signingKey = base64.b64encode(
        hmac.new(ncloud_secretkey2, message, digestmod=hashlib.sha256).digest()
    )
    if method == "GET":
        ...
        http_header = {
            "x-ncp-apigw-timestamp": var.timestamp,
            "x-ncp-iam-access-key": api_key.ncloud_gov_accesskey,
            "x-ncp-apigw-signature-v2": signingKey,
        }
